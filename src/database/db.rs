...
use super::{
    schema::{
        simple_sudoku_canonical::dsl::*,
        simple_sudoku_canonical_squares::dsl::*,
...
impl Database {
    ...
    pub fn get_all_simple_sudoku_canonical_squares(
        ...
        simple_sudoku_canonical_squares.get_results::<DBSimpleSudokuCanonicalSquares>(
            &mut self.connection
        )
    }
    ...
    pub fn get_n_simple_sudoku_game(
        ...
        simple_sudoku_games.limit(n).get_results::<DBSimpleSudokuGame>(&mut self.connection)
    }
    ...
    pub fn insert_simple_sudoku_canonical(
        ...
        diesel::insert_into(simple_sudoku_canonical).values(&sudoku).execute(&mut self.connection)?;
        diesel
            ::insert_into(simple_sudoku_canonical_squares)
            .values(&squares)
    ...
    pub fn insert_simple_sudoku_game(
        ...
        diesel::insert_into(simple_sudoku_games).values(&sudoku).execute(&mut self.connection)
    }
    ...
    pub fn insert_multiple_simple_sudoku_canonical(
        ...
        for sudokus_chunk in sudokus.chunks(16348) {
            inserted_sudokus += diesel
                ::insert_into(simple_sudoku_canonical)
    ...
    pub fn insert_multiple_simple_sudoku_game(
        ...
        for sudokus_chunk in sudokus.chunks(16348) {
            inserted_sudokus += diesel
                ::insert_into(simple_sudoku_games)
                .values(sudokus_chunk)
                .execute(&mut self.connection)?;
        }
        Ok(inserted_sudokus)
