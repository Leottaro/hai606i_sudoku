...
fn carpet_filled(max_number: usize) {
    ...
    //         .spawn(move || {
    //             while *remaining_number.lock().unwrap() > 0 {
    //                 let carpet_base = CarpetSudoku::generate_full(
    //                     3,
    //                     thread_patterns_loop.lock().unwrap().next().unwrap()
    //                 );
    ...
    //         ): ((Vec<_>, Vec<_>), (Vec<_>, Vec<_>)) = locked.drain(0..drain_number).unzip();
    //         (
    //             carpets,
    ...
    //     remaining_number.lock().unwrap().sub_assign(just_inserted_carpets);
    //     total_count += total_rows as u128;
...
fn carpet_games(max_number: usize) {
    ...
    //         .lock()
    //         .unwrap()
    //         .get_n_canonical_carpets(max_number as i64)
    //         .unwrap();
    //     (temp.len(), temp.into_iter())
    // };
    ...
    // for canonical in canonicals {
    //     remaining_canonicals.sub_assign(1);
    //     let sudoku = CarpetSudoku::db_from_filled(canonical, sudokus);
    //     let mut passed_games = Vec::new();
    //     println!("{} canonicals left:{}", remaining_canonicals, " ".repeat(50));
