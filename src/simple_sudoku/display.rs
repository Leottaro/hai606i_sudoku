...
impl<'a> SudokuDisplay<'a> {
    ...
    async fn draw_buttons(&self, font: Font) {
        ...
        for x in 0..self.sudoku.get_n() {
            for y in 0..self.sudoku.get_n() {
                ...
                let b_padding = 10.0;
                let b_x =
                    self.x_offset + self.window_size + 150.0 + (x as f32) * (b_size + b_padding);
                let b_y = self.y_offset
                    + (self.window_size - (b_size + b_padding) * (self.sudoku.get_n() as f32))
                        / 2.0
    ...
    pub async fn run(&mut self, font: Font) {
        ...
        if is_mouse_button_pressed(MouseButton::Left) {
            ...
            let b_y = self.y_offset
                + (self.window_size - (b_size + b_padding) * (self.sudoku.get_n() as f32)) / 2.0;
        ...
        if (mouse_x as f32) < self.window_size
            && (mouse_x as f32) > 0.0
            && (mouse_y as f32) < self.window_size
            && (mouse_y as f32) > 0.0
        {
            if is_mouse_button_pressed(MouseButton::Left) {
